...
class Encoder(nn.Module):
    ...
    def forward(self, x, edge_index, batch, percent):
        ...
        # return pos_global_graph, sub, sub_graph
...
class GraphEnhance(nn.Module):
    ...
    def forward(self, x, edge_index, batch, percent):
        ...
        # pos_global_graph, sampling_subgraph, neg_global_graph, prob_loss, infonce = self.encoder(x,edge_index,batch,percent)
        # =========================================
        ...
        graph, pos_subgraph, neg_subgraph = self.encoder(x, edge_index, batch, percent)


        sub_list = [i for i in range(0, (int(batch[-1]) + 1))]
        # sub_batch = [val for val in sub_list for _ in range(2 ** self.times)]
        sub_batch = torch.tensor([val for val in sub_list for _ in range(2 ** self.times)]).cuda()

        measure = 'JSD'
        local_global_loss = local_global_loss_(graph, pos_subgraph, sub_batch, measure)
        global_global_loss = global_global_loss_(graph, neg_subgraph, batch, measure)  #
